...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        {
            if (PWBSettings.shortcuts.gridEnableShortcuts.keyCombination.Check())
            {
                if (!_gridShorcutEnabled) _gridShorcutEnabled = true;
            }
            if (_gridShorcutEnabled)
            {
                if(PWBSettings.shortcuts.gridToggle.keyCombination.Check())
                {
                    SnapManager.settings.visibleGrid = !SnapManager.settings.visibleGrid;
                    _gridShorcutEnabled = false;
                }
                else if (PWBSettings.shortcuts.gridToggleSnaping.keyCombination.Check())
                {
                    SnapManager.settings.snappingEnabled = !SnapManager.settings.snappingEnabled;
                    _gridShorcutEnabled = false;
                }
                else if (PWBSettings.shortcuts.gridToggleLock.keyCombination.Check())
                {
                    SnapManager.settings.lockedGrid = !SnapManager.settings.lockedGrid;
                    _gridShorcutEnabled = false;
                }
                else if (PWBSettings.shortcuts.gridSetOriginPosition.keyCombination.Check()
                ...
                else if (PWBSettings.shortcuts.gridSetOriginRotation.keyCombination.Check()
                    && UnityEditor.Selection.activeTransform != null)
                {
                ...
                else if (PWBSettings.shortcuts.gridSetSize.keyCombination.Check()
                    && UnityEditor.Selection.activeTransform != null)
                {
