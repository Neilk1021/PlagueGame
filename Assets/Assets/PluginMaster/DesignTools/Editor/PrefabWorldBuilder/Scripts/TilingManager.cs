...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        private static void TilingToolEditMode(UnityEditor.SceneView sceneView)
        {
            ...
            else if (PWBSettings.shortcuts.editModeSelectParent.keyCombination.Check()
            ...
            else if (PWBSettings.shortcuts.editModeDeleteItemButNotItsChildren.keyCombination.Check())
                TilingManager.instance.DeletePersistentItem(_selectedPersistentTilingData.id, false);
            else if (PWBSettings.shortcuts.editModeDeleteItemAndItsChildren.keyCombination.Check())
        ...
        private static bool TilingShortcuts(TilingData data)
        {
            ...
            if (PWBSettings.shortcuts.selectionRotate90XCCW.keyCombination.Check())
            {
                RotateTiling(data, 90, Vector3.right, true);
                return true;
            }
            if (PWBSettings.shortcuts.selectionRotate90XCW.keyCombination.Check())
            {
                RotateTiling(data, 90, Vector3.left, true);
                return true;
            }
            if (PWBSettings.shortcuts.selectionRotate90YCCW.keyCombination.Check())
            {
                RotateTiling(data, 90, Vector3.up, true);
                return true;
            }
            if (PWBSettings.shortcuts.selectionRotate90YCW.keyCombination.Check())
            {
                RotateTiling(data, 90, Vector3.down, true);
                return true;
            }
            if (PWBSettings.shortcuts.selectionRotate90ZCCW.keyCombination.Check())
            {
                RotateTiling(data, 90, Vector3.forward, true);
                return true;
            }
            if (PWBSettings.shortcuts.selectionRotate90ZCW.keyCombination.Check())
            {
